...
app.Game = function(elem) {
  ...
  this.tutorial = new app.shared.Tutorial('presentbounce_drag.mp4 presentbounce_conveyor.mp4 presentbounce_go.mp4');
...
/**
 * Starts the game.
 * @param {?{level: string}} data
 * @export
 */
app.Game.prototype.start = function(data=null) {
  // Bind listener to scale scene when window resizes
  this.watchSceneSize_();
  this.restart(data && +data.level || 1);
  this.tutorial.start();
};
