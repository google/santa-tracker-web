...
THREE.GLTFLoader = (function() {
  ...
  GLTFLightsExtension.prototype.loadLight = function(lightIndex) {
    ...
    switch (lightDef.type) {
      ...
      case 'spot':
        ...
        lightDef.spot.outerConeAngle = lightDef.spot.outerConeAngle !== undefined ?
  ...
  function GLTFBinaryExtension(data) {
    ...
    while (chunkIndex < chunkView.byteLength) {
      ...
      if (chunkType === BINARY_EXTENSION_CHUNK_TYPES.JSON) {
        var contentArray =
            new Uint8Array(data, BINARY_EXTENSION_HEADER_LENGTH + chunkIndex, chunkLength);
  ...
  function GLTFMaterialsPbrSpecularGlossinessExtension() {
    return {
      ...
      extendParams: function(materialParams, materialDef, parser) {
        ...
        var fragmentShader =
            shader.fragmentShader.replace('uniform float roughness;', 'uniform vec3 specular;')
      ...
      refreshUniforms: function(renderer, scene, camera, geometry, material) {
        ...
        if (material.envMap) {
          ...
          //  WebGLRenderTargetCube.texture will be flipped because it's a Texture and NOT a
          //  CubeTexture
          // this check must be handled differently, or removed entirely, if WebGLRenderTargetCube
          // uses a CubeTexture in the future
  ...
  var INTERPOLATION = {
    CUBICSPLINE: undefined,  // We use a custom interpolant (GLTFCubicSplineInterpolation) for
                             // CUBICSPLINE tracks. Each keyframe track will be initialized with a
  ...
  GLTFParser.prototype.loadAccessor = function(accessorIndex) {
    ...
    return Promise.all(pendingBufferViews).then(function(bufferViews) {
      ...
      if (byteStride && byteStride !== itemBytes) {
        // Each "slice" of the buffer, as defined by 'count' elements of 'byteStride' bytes, gets
        // its own InterleavedBuffer This makes sure that IBA.count reflects accessor.count properly
      ...
      if (accessorDef.sparse !== undefined) {
        ...
        var sparseIndices = new TypedArrayIndices(
            bufferViews[1], byteOffsetIndices, accessorDef.sparse.count * itemSizeIndices);
        var sparseValues =
            new TypedArray(bufferViews[2], byteOffsetValues, accessorDef.sparse.count * itemSize);

        if (bufferView !== null) {
          // Avoid modifying the original ArrayBuffer, if the bufferView wasn't initialized with
          // zeroes.
          bufferAttribute = new THREE.BufferAttribute(
              bufferAttribute.array.slice(), bufferAttribute.itemSize, bufferAttribute.normalized);
        }
  ...
  GLTFParser.prototype.assignTexture = function(materialParams, mapName, mapDef) {
    ...
    return this.getDependency('texture', mapDef.index).then(function(texture) {
      ...
      if (parser.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]) {
        ...
        if (transform) {
          texture =
              parser.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM].extendTexture(texture, transform);
        }
      }
    ...
    if (materialExtensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]) {
      ...
      if (metallicRoughness.metallicRoughnessTexture !== undefined) {
        pending.push(parser.assignTexture(
            materialParams, 'metalnessMap', metallicRoughness.metallicRoughnessTexture));
        pending.push(parser.assignTexture(
            materialParams, 'roughnessMap', metallicRoughness.metallicRoughnessTexture));
      }
    }
  ...
  function addPrimitiveAttributes(geometry, primitiveDef, parser) {
    ...
    if (primitiveDef.indices !== undefined && !geometry.index) {
      var accessor =
          parser.getDependency('accessor', primitiveDef.indices).then(function(accessor) {
      ...
      return parser.loadGeometries(primitives).then(function(geometries) {
        ...
        for (var i = 0, il = geometries.length; i < il; i++) {
          ...
          if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
              ...
              primitive.mode === WEBGL_CONSTANTS.TRIANGLE_FAN || primitive.mode === undefined) {
            // .isSkinnedMesh isn't in glTF spec. See .markDefs()
            mesh = meshDef.isSkinnedMesh === true ? new THREE.SkinnedMesh(geometry, material) :
                                                    new THREE.Mesh(geometry, material);

            if (mesh.isSkinnedMesh === true && !mesh.geometry.attributes.skinWeight.normalized) {
              // we normalize floating point skin weight array to fix malformed assets (see #15319)
              // it's important to skip this for non-float32 data since normalizeSkinWeights assumes
  ...
  GLTFParser.prototype.loadAnimation = function(animationIndex) {
    ...
    for (var i = 0, il = animationDef.channels.length; i < il; i++) {
      ...
      var input = animationDef.parameters !== undefined ? animationDef.parameters[sampler.input] :
                                                          sampler.input;
      var output = animationDef.parameters !== undefined ? animationDef.parameters[sampler.output] :
                                                           sampler.output;
            ...
            for (var j = 0, jl = targetNames.length; j < jl; j++) {
              ...
              if (sampler.interpolation === 'CUBICSPLINE') {
                track.createInterpolant = function InterpolantFactoryMethodGLTFCubicSpline(result) {
                  ...
                  // representing inTangent, splineVertex, and outTangent. As a result,
                  // track.getValueSize() must be divided by three to get the interpolant's
                  // sampleSize argument.
                ...
                // Mark as CUBICSPLINE. `track.getInterpolation()` doesn't support custom
                // interpolants.
    ...
    return (function() {
             ...
             if (nodeDef.extensions && nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL] &&
               ...
               pending.push(parser.getDependency(
                   'light', nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light));
             }
