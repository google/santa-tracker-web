...
Blockly.BlockSvg = function(block) {
  ...
  this.svgPathDark_ = Blockly.createSvgElement('path',
      {'class': 'blocklyPathDark', 'transform': 'translate(0, 5)'},
      this.svgGroup_);
  ...
  this.svgPathTouch_ = Blockly.createSvgElement('path',
      {'class': 'blocklyPathTouch', 'transform': 'translate(8, 14)',
          'd': Blockly.BlockSvg.TOUCH_PATH},
      this.svgGroup_);
 ...
 * @const
 */
Blockly.BlockSvg.MIN_BLOCK_Y = 42;
/**
...
/**
 * Radius of vertical tab.
 */
Blockly.BlockSvg.NOTCH_RADIUS = 8;
/**
 ...
 * @const
 */
Blockly.BlockSvg.FIELD_HEIGHT = 26;
/**
 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_LEFT = 'a ' + Blockly.BlockSvg.NOTCH_RADIUS + ',' +
    Blockly.BlockSvg.NOTCH_RADIUS + ' 0 0,1 ' +
    (Blockly.BlockSvg.NOTCH_RADIUS * 2) + ',0';
/**
 * SVG path for drawing next/previous notch from left to right with
 * highlighting.
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_LEFT_HIGHLIGHT = 'a ' + Blockly.BlockSvg.NOTCH_RADIUS + ',' +
    Blockly.BlockSvg.NOTCH_RADIUS + ' 0 0,1 ' +
    (Blockly.BlockSvg.NOTCH_RADIUS * 2) + ',0';
/**
 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_RIGHT = 'a ' + Blockly.BlockSvg.NOTCH_RADIUS + ',' +
    Blockly.BlockSvg.NOTCH_RADIUS + ' 0 1,0 ' +
    (-Blockly.BlockSvg.NOTCH_RADIUS * 2) + ',0';
/**
 * SVG path for drawing a highlight for a notch.
 * @type {string}
 */
Blockly.BlockSvg.NOTCH_HIGHLIGHT = 'm ' +
    (-Blockly.BlockSvg.NOTCH_RADIUS * 2 - 5) +
    ',0 h 5 ' + Blockly.BlockSvg.NOTCH_PATH_LEFT + ' h 5';
/**
 * SVG path for drawing a highlight for a notch in RTL locale.
 * @type {string}
 */
Blockly.BlockSvg.NOTCH_HIGHLIGHT_RTL = 'm -5,0 h 5 ' +
...
Blockly.BlockSvg.TOP_RIGHT_CORNER_HIGHLIGHT =
    'a ' + (Blockly.BlockSvg.CORNER_RADIUS - 1) + ',' +
    (Blockly.BlockSvg.CORNER_RADIUS - 1) + ' 0 0,1 ' +
    Blockly.BlockSvg.CORNER_RADIUS + ',' + (Blockly.BlockSvg.CORNER_RADIUS + 1);
/**
...
Blockly.BlockSvg.INNER_TOP_LEFT_CORNER =
    ...
    (Blockly.BlockSvg.NOTCH_WIDTH - Blockly.BlockSvg.NOTCH_RADIUS * 2 - Blockly.BlockSvg.CORNER_RADIUS) +
    ' a ' + Blockly.BlockSvg.CORNER_RADIUS + ',' +
...
Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_LTR =
    ...
    (Blockly.BlockSvg.DISTANCE_45_OUTSIDE + 1);
/**
 * SVG path for drawing the touch nub.
 * @const
 */
Blockly.BlockSvg.TOUCH_PATH =
    'M13.5,3h-12C0.7,3,0,2.3,0,1.5S0.7,0,1.5,0h12C14.3,0,15,0.7,15,1.5S14.3,3,13.5,3z' +
    'M15,7.5C15,6.7,14.3,6,13.5,6h-12C0.7,6,0,6.7,0,7.5C0,8.3,0.7,9,1.5,9h12C14.3,9,15,8.3,15,7.5z' +
    'M15,13.5c0-0.8-0.7-1.5-1.5-1.5h-12C0.7,12,0,12.7,0,13.5S0.7,15,1.5,15h12C14.3,15,15,14.3,15,13.5z';
...
Blockly.BlockSvg.prototype.disposeUiEffect = function() {
  ...
  Klang.triggerEvent('computer_trash');
...
Blockly.BlockSvg.prototype.connectionUiEffect = function() {
  ...
  Klang.triggerEvent('computer_connect');
...
Blockly.BlockSvg.prototype.updateColour = function() {
  ...
  var hexColour = this.block_.getHexColour();
  var rgb = goog.color.hexToRgb(hexColour);
  // Don't need svgPathLight for Santatracker
  ...
  this.svgPathTouch_.setAttribute('fill', goog.color.rgbArrayToHex(rgbDark));
  var pattern = this.block_.getFillPattern();
  if (pattern) {
...
Blockly.BlockSvg.prototype.rowIndent = function() {
  if (Blockly.touch && !this.block_.mini) {
    return Blockly.BlockSvg.SEP_SPACE_X * 3.5;
  } else {
    return Blockly.BlockSvg.SEP_SPACE_X;
  }
};
...
Blockly.BlockSvg.prototype.renderCompute_ = function(iconWidth) {
  ...
  for (var i = 0, input; input = inputList[i]; i++) {
    ...
    if (input.connection && input.connection.targetConnection) {
      ...
      // Make room for the shadow, and then some.
      input.renderHeight = Math.max(input.renderHeight, bBox.height + 4 + 3);
  ...
  if (hasValue) {
    inputRows.rightEdge = Math.max(inputRows.rightEdge, fieldValueWidth +
        this.rowIndent() + Blockly.BlockSvg.SEP_SPACE_X + Blockly.BlockSvg.TAB_WIDTH);
  } else if (hasDummy) {
    inputRows.rightEdge = Math.max(inputRows.rightEdge, fieldValueWidth +
        this.rowIndent() + Blockly.BlockSvg.SEP_SPACE_X);
  }
...
//    }
  ...
  this.svgPathTouch_.setAttribute('display',
      Blockly.touch && !this.block_.mini ? '' : 'none');
  if (Blockly.RTL) {
...
Blockly.BlockSvg.prototype.renderDrawTop_ =
  ...
  if (this.block_.previousConnection) {
    steps.push('H', Blockly.BlockSvg.NOTCH_WIDTH -
        Blockly.BlockSvg.NOTCH_RADIUS * 2);
  ...
  highlightSteps.push('H', rightEdge - Blockly.BlockSvg.CORNER_RADIUS +
      (Blockly.RTL ? -1 : 0));
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(steps, highlightSteps,
  ...
  for (var y = 0, row; row = inputRows[y]; y++) {
    ...
    if (this.block_.isCollapsed()) {
      ...
      this.renderDrawRightEdge_(steps, highlightSteps, cursorY + remainder,
      ...
      this.renderDrawRightEdge_(cursorY + row.height, false);
      ...
      this.renderFields_(input.fieldRow, fieldX, fieldY);
      this.renderDrawRightEdge_(steps, highlightSteps, cursorY + row.height,
                                y === inputRows.length - 1);
      ...
      if (y == inputRows.length - 1 ||
        ...
        this.renderDrawRightEdge_(steps, highlightSteps,
            cursorY + row.height + Blockly.BlockSvg.SEP_SPACE_Y,
            y === inputRows.length - 1);
        cursorY += Blockly.BlockSvg.SEP_SPACE_Y;
      }
    }
  ...
  if (!inputRows.length) {
    ...
    this.renderDrawRightEdge_(steps, highlightSteps, cursorY, true);
  }
  return cursorY;
};

/**
 * Render one of potentially many right edges, with optional rounding
 * for the last corner.
 * @param {!Array.<string>} steps Path of block outline.
 * @param {!Array.<string>} highlightSteps Path of block highlights.
 * @param {number} targetY Draw edge to Y position.
 * @param {boolean} isRounded Draw a bottom-right rounded corner.
 * @private
 */
Blockly.BlockSvg.prototype.renderDrawRightEdge_ = function(steps, highlightSteps, targetY, isRounded) {
  if (isRounded) {
    steps.push('V', targetY - Blockly.BlockSvg.CORNER_RADIUS);
    steps.push(Blockly.BlockSvg.BOTTOM_RIGHT_CORNER);
    if (Blockly.RTL) {
      highlightSteps.push('V', targetY - Blockly.BlockSvg.CORNER_RADIUS - 1);
    }
  } else {
    steps.push('V', targetY);
    if (Blockly.RTL) {
      highlightSteps.push('V', targetY - 1);
    }
  }
};
