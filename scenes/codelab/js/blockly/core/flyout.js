...
/**
 * Allows external control of flyout layout.
 * @type {Function}
 */
Blockly.Flyout.prototype.layoutProvider = null;
...
/**
 * Whitespace between flyout elements.
 * @type {number}
 * @const
 */
Blockly.Flyout.prototype.WHITESPACE = 8;
...
Blockly.Flyout.prototype.getMetrics_ = function() {
  ...
  var viewHeight = this.height_ - 2 * this.WHITESPACE;
...
/**
 * Runs an external layout system.
 * @private
 */
Blockly.Flyout.prototype.externalReflow_ = function() {
  ...
  if (this.width_ != flyoutWidth) {
    ...
    // Update main workspace position.
    Blockly.mainWorkspace.scrollX = this.width_;
    if (Blockly.RTL) {
  ...
  return function(e) {
    ...
    if (Blockly.isRightButton(e)) {
      ...
      Blockly.Flyout.onMouseUpWrapper_ = Blockly.bindEvent_(document,
          'mouseup', flyout, flyout.onMouseUpBlock_);
...
/**
 * Fires an event notifying interested parties that a flyout block was
 * clicked instead of dragged.
 * @private
 */
Blockly.Flyout.prototype.onMouseUpBlock_ = function() {
  Blockly.fireUiEvent(this.workspace_.getCanvas(), 'blocklyClickFlyoutBlock');
  Blockly.terminateDrag_();
};
...
Blockly.Flyout.prototype.createBlockFunc_ = function(originBlock) {
  ...
  return function(e) {
    ...
    var xml = originBlock.getCloneBlock();
    if (xml == null) {
      xml = Blockly.Xml.blockToDom_(originBlock);
    }
