...
app.Viewport = function(elem, blocksElem, blocksContainerElem) {
  ...
  this.debouncedResize_ = app.utils.debounce(this.onWindowResize_.bind(this), app.Constants.RESIZE_DEBOUNCE_THRESHOLD_MS);
};
...
app.Viewport.prototype.attachEvents_ = function() {
  $(window).on('resize', this.debouncedResize_);

  this.controlsElem.on('click', this.onPaginateClick_.bind(this));
  this.controlsElem.on('touchstart', this.onPaginateTouch_.bind(this));

  this.elem.on('touchstart', this.onTouchSceneStart_.bind(this));
  this.elem.on('touchmove', this.onTouchSceneMove_.bind(this));
  this.elem.on('touchend', this.onTouchSceneEnd_.bind(this));
};
...
app.Viewport.prototype.detachEvents_ = function() {
  $(window).off('resize', this.debouncedResize_);

  // Assume ownership of controlsElem and elem, so remove all listeners for
  // these events.

  this.controlsElem.off('click');
  this.controlsElem.off('touchstart');
 ...
 * @private
 */
app.Viewport.prototype.onTouchSceneStart_ = function(event) {
 ...
 * @private
 */
app.Viewport.prototype.onTouchSceneMove_ = function(event) {
 ...
 * @private
 */
app.Viewport.prototype.onTouchSceneEnd_ = function(event) {
  var touchObj = event.originalEvent.changedTouches[0];
 ...
 * @private
 */
app.Viewport.prototype.onPaginateTouch_ = function(event) {
 ...
 * @private
 */
app.Viewport.prototype.onPaginateClick_ = function(event) {
  this.handlePaginate(parseInt($(event.currentTarget).data('direction'), 10));
};
 ...
 *
 * We need to update our elements width based on the new screen size and update
 * the container accordingly.
 *
 * @private
 */
app.Viewport.prototype.onWindowResize_ = function() {
  // Update our elements
  this.calculateWidth();
  this.setStyles();
...
/**
 * Gets the current viewport width times the factor of how much each block
 * should occupy.
 */
...
/**
 * Sets the width and left positions of our container and its child blocks.
 */
app.Viewport.prototype.setStyles = function() {
  var i = null;

  // Update the container width
  this.blocksContainerElem.css('width', this.blocksContainerWidth);
...
app.Viewport.prototype.getBlockPosition = function() {
  ...
  if (this.blocksContainerWidth === app.Constants.FULL_SCENE_MIN_WIDTH) {
    ...
    value = (this.blocksContainerWidth - this.viewportWidth) / 2;
  } else {
    value = this.blockWidth - (this.blockWidth * app.Constants.VIEW_GAP);
  }
...
/**
 * Positions the container by translating its value.
 * @param {boolean} forceTransition whether to update without animation
 */
app.Viewport.prototype.updateContainerPosition = function(forceTransition) {
  ...
  window.setTimeout(function() {
    this.isAnimating = false;
  }.bind(this), app.Constants.CONTAINER_TRANSITION_DURATION_MS);
};


/**
 * Performs a very subtle parallax on the mountain, by moving it at the same
 * time as the container is moving.
 * @param {boolean} forceTransition whether to update without animation
 */
app.Viewport.prototype.moveMountain = function(forceTransition) {
  ...
  // If it's an instant update, there's no need to parallax the mountain
  if (forceTransition) {
    return;
  }
