 ...
 */
app.world.UserObject = class extends app.world.LevelObject {
  ...
  getMouseWorldVector(mouseX, mouseY) {
    ...
    const offsetX = viewport.sceneOffset.left + (windowWidth - app.Constants.CANVAS_WIDTH*scale) / 2;
    const offsetY = viewport.sceneOffset.top + (windowHeight - app.Constants.CANVAS_HEIGHT*scale) / 2;

    const x = (mouseX - offsetX) / scale;
    const y = (mouseY - offsetY) / scale;

    return new b2.Vec2(app.Unit.toWorld(x), app.Unit.toWorld(y));
  }
  ...
  createMouseJoint_(e) {
    ...
    def.target = this.getMouseWorldVector(e.clientX || 0, e.clientY || 0)
