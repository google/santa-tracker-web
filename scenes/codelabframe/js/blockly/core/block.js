...
Blockly.Block.prototype.onMouseUp_ = function(e) {
  ...
  Blockly.doCommand(function() {
    Blockly.terminateDrag_();
    Klang.triggerEvent('computer_place');
  ...
  Blockly.doCommand(function() {
    ...
    if (Blockly.Block.dragMode_ == 1) {
      ...
      if (dr > Blockly.DRAG_RADIUS) {
        ...
        Blockly.fireUiEvent(this_.workspace.getCanvas(), 'blocklyDragBlock');
        Klang.triggerEvent('computer_pickup');
      }
    }
    if (Blockly.Block.dragMode_ == 2) {
    ...
    // This event has been handled.  No need to bubble up to the document.
    e.stopPropagation();
    // No need to switch tabs either. (iOS, I'm looking at you!)
    e.preventDefault();
  });
};
...
/**
 * Sets the block XML which should be used when cloning this block, e.g. by
 * dragging it from the toolbox.
 * @param {!Element} newCloneBlock to use.
 */
Blockly.Block.prototype.setCloneBlock = function(newCloneBlock) {
  this.cloneBlock_ = newCloneBlock;
};

/**
 * Return block XML which should be used when cloning this block, e.g. by
 * dragging it from the toolbox.
 * @return {Blockly.Block} The block to clone.
 */
Blockly.Block.prototype.getCloneBlock = function() {
...
/**
 * Set whether block should be rendered in a minimized state.
 * @param {boolean} newBoolean True if block should be mini.
 */
Blockly.Block.prototype.setMini = function(newBoolean) {
  this.mini = newBoolean;
  if (this.rendered) {
    this.render();
    this.bumpNeighbours_();
    this.workspace.fireChangeEvent();
